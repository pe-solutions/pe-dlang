...
private bool isSpecial(ref int[7] a) pure nothrow @nogc {
    // Property (ii): sum of k+1 smallest > sum of k largest, for k = 1, 2, 3
    foreach (k; 1 .. 4) {
        int lo = 0, hi = 0;
        foreach (i; 0 ..     k + 1) lo += a[i];
        foreach (i; 7 - k .. 7    ) hi += a[i];
        if (lo <= hi) return false;
    }
    // Property (i): all 127 non-empty subset sums are distinct
    bool[512] seen;
